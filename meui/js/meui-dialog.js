...
//	$.fn.extend({
	...
	var meuiDialog = {
		notice:function(options){
			...
			//
			var weuiParent = '.section-meuiDialog-notice',
		...
		* MEUI弹出窗
		...
		alert:function(options){
			...
			var shade = settings.mask,
					...
					message = typeof settings.message == 'undefined' || settings.message == '欢迎使用MEUI对话框插件！' ? '' : settings.message,
			...
			var _html = '<div class="' + IdClass + '"' + nodeStr + ' data-maskId="' + shade + '"' + _alertStyle + '>'+
						'	<div class="alert-layout'+themeClass+'">'+
								'<div class="alert-close meuiDialog-close"'+crossStyle+'></div>'+
		...
		* MEUI输入框
		...
		feedback:function(options){
			...
			var defaults = {
				...
				openBack: null, //窗口加载完成后的回调函数. 参数：e 控件根节点jq对象
				callBack: null, //点按钮后的回调函数(可选). 参数：ret 按钮索引值, e 控件根节点jq对象
			...
			var _html = '<div class="'+IdClass+'" id="' + parentId.replace(/[\#\.]/g,'') + '" data-maskId="' + shade + '">'+
								...
								'		<div class="feedback-close feedback-back meuiDialog-back"'+backStyle+'>' + backText + '</div>'+ // add 20210709-1
									'		<div class="feedback-close meuiDialog-close"'+crossStyle+'></div>'+
			...
			if(popup){ //...允许弹窗
				...
				if(animate){ //有转圈
					...
					setTimeout(function(){
						var ret = bubbleRet; //默认执行哪个按钮里的事件
						if(settings.callBack) settings.callBack(ret, $(parentId)); //edit 20210327-1
						self.destroyAnimate(); //销毁转圈
					},loadDelay)
				}else{
					var ret = bubbleRet; //默认执行哪个按钮里的事件
			...
			$(document).off('click',parentId + ' .feedback-button-group>.feedback-btn').on('click',parentId + ' .feedback-button-group>.feedback-btn',function(){
				...
				var ret = $(this).index()+1;
				if(settings.callBack){
					settings.callBack(ret, $(parentId)); //edit 20210327-1
				}
			...
			function openFeedback(){
				...
				setTimeout(function(){ //延迟一下,防止获取的宽、高有误
					...
					if(settings.openBack){
						settings.openBack($(parentId)); //edit 20210327-1
					}
		...
		closeFeedWindow:function(){
			$('.section-meuiDialog-feedback').next('.meuiDialog-mask').remove();
		...
		*/
		...
		generateMaskNode:function(){
			var str = 'meuiDialog-mask'; //请不要添加井号#或者点号.
			return str;
		},
		...
		*/
		showAnimate:function(str){
			...
			var _html='<div class="meuiDialog-loading">'+
						'	<div class="meuiDialog-animate"><i class="icon icon-load"></i>'+spanTxt+'</div>'+ //edit 20180616-1
						'</div>';
			if($('.meuiDialog-loading').length==0) $('body').append(_html);
		},
		...
		*/
		preventWinRoll: function(){
			...
			$.meuiFeedLayScroll($('.section-meuiDialog-notice'), '.help-layout');
			$.meuiFeedLayScroll($('.section-meuiDialog-alert'), '.alert-layout');
			$.meuiFeedLayScroll($('.section-meuiDialog-feedback'), '.feedback-content');
			$.meuiFeedLayScroll($('.meuiDialog-mask'), '.meuiDialog-mask');
		},
	...
	*/
	$.meuiFeedLayScroll = function(container, selectorScrollable) {
