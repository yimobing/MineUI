 ...
 */
 ...
 (function($){
    $.fn.extend({
        meuiSearchBox:function(options){
            var $this = this;
            var defaults = {
                inputBox:{},
                searchButton:{}
            ...
            //私有成员
            $.extend({
                searchRootElement:'.meui-searchbox', //根节点 
           ...
           var  parentClassID = rootNode.replace(/#|\./g,''), //去掉井号#或点号.
            ...
            $.each(inputBox.group,function(i,items){
                ...
                if(type.indexOf('radio') < 0){
                    ...
                    if(type.indexOf('checkbox') >= 0){
                        ...
                        if(_cClass != '') iClassName += ' meui-checkbox' + _cClass + '';
                    ...
                    var _rClassStr = _rClass == '' ? '' : ' class="meui-radio' + _rClass + '"';
            ...
            var seaHeadClass = 'meui-search-head';
                    ...
                    function checkFormComplete(must,value,tips){ 
                        if(must!="0" && value==''){
                            if(typeof meuiDialog!='undefined'){
                                if(typeof meuiDialog.alert === 'function'){
                                    meuiDialog.alert({
                                        caption:'提示',
                                        message:tips,
                                        buttons:["确定"]
                ...
                $this.find(colNode).find(boxNode).find('input:text').each(function(){
                    ...
                    if(type.indexOf('date') >= 0){
                        if(typeof meuiCalendar == 'undefined'){
                            console.log('“' + wenzi + '” 需引入日历控件：jquery.jedate.js, meui-calendar.js');
                            return;
                        } 
                        var value = $(this).val(); //先取值
                        meuiCalendar.jeCalendar($(this), false); //再调用日历控件
                        //if(value==''){
                            var dates = parent.attr('data-date');
                            if(dates=='当天') assignElementValue($(this),meuiCalendar.getNowtime());
                            if(dates=='本月初') assignElementValue($(this),meuiCalendar.getMonthFirstDay());
                            if(dates=='本月末') assignElementValue($(this),meuiCalendar.getMonthLastDay());
                            if(dates=='本季初') assignElementValue($(this),meuiCalendar.getQuarterStartDay());
                            if(dates=='本季末') assignElementValue($(this),meuiCalendar.getQuarterEndDay());
                            if(dates=='本年初') assignElementValue($(this),meuiCalendar.getYearFirstDay());
                            if(dates=='本年末') assignElementValue($(this),meuiCalendar.getYearLastDay());
                            if(dates=='本年度') assignElementValue($(this),meuiCalendar.getYear());
                            if(dates=='本月份') assignElementValue($(this),meuiCalendar.getMonth());
                            if(dates=='本日') assignElementValue($(this),meuiCalendar.getDay());
                        //}
                    }
                    //2.需要click才触发
                    _this.on('click',function(){
                        ...
                        if(type.indexOf('drop') >= 0){
                            if(typeof meuiDropdown == 'undefined'){
                                console.log('“' + wenzi + '” 需引入下拉控件：meui-dropdown.js');
                                return;
                            } 
