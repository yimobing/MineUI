...
function meuiInputsearch(options, input) {
	var defaults = {
	...
	var _closeStr = !settings.closeButton ? '' : '<div id="meuiInputSearch-close">关闭</div>';
	...
	function setHeight() {
		...
		var selfH = $('#searchUlNew').outerHeight(true) + $('#meuiInputSearch-close').outerHeight(true); //自身高度
	...
	var matchSpring = function () {
		...
		if (arr.length > 0) {
			...
			$('#meuiInputSearch-zeroData').remove();
		} else {
			if(settings.noDataDrop) {
				$('#searchListNew').prepend('<div id="meuiInputSearch-zeroData">无数据</div>');
				$('#searchUlNew').hide();
			}
			else $('#meuiInputSearch-close').remove();
		}
	}
	...
	$("#meuiInputSearch-close").click(function (e) {
		e.stopPropagation();
		$('#searchUlNew').html("");
		$('#searchListNew').remove();
		quer = 1;
