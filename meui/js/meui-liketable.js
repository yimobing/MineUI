...
(function($){
	...
	var tablePC = function(){
		...
		return {
			init:function(options){
				...
				var nodataNode = '.meui-like-nodata';
	 ...
	 * @param {object} ps_params 插件内部参数(可选). 
	 * 本参数可用于:
	...
	var createBookList = function(ps_opt, ps_params){
		...
		$.each(datasource.data, function(i, items){ //循环数据(行)
			...
			for(var j = 0; j < columnsArr.length; j++){ //循环配置(列)
				...
				if(mode.indexOf('radio') >= 0){ //radio单选
					...
					if(mode.indexOf('tick') >= 0 || mode.indexOf('on-off') >= 0){ //单选“打勾”
						...
						_editBoxStr = '<div class="cell-radio-single">'+
										'<input type="checkbox" class="meui-checkbox' + _shapeClass + _colorClass + ' i-t-' + field + _nameClass + '"' + _readonlyStr + _disabledStr + _checkedStr + '>'+
									 '</div>';
					}
					if(mode.indexOf('whether') >= 0){ //单选“是否"
						...
						_editBoxStr = '<div class="cell-radio-group">'+
										'<div class="group">'+
											'<input type="radio" name="' + field + '-' + (i + 1) + '" id="' + field + '-yes-' + (i + 1) + '" class="meui-radio green' + _nameClass + '" value="' + yesVal + '"' + _checkYesStr + _readonlyStr + _disabledStr + '>'+
											'<label for="' + field + '-yes-' + (i + 1) + '">' + yesText + '</label>'+
										'</div>'+
										'<div class="group">'+
											'<input type="radio" name="' + field + '-' + (i + 1) + '" id="' + field + '-no-' + (i + 1) + '" class="meui-radio green' + _nameClass + '" value="' + noVal + '"' + _checkNoStr + _readonlyStr + _disabledStr + '>'+
				...
				//列表HTML
				if(j == 0){ //前面列
					if(isCheckboxCol) _bodyHTML += '<div class="col-checkbox center" style="width:50px; min-height:20px;"><input type="checkbox" class="meui-checkbox square-tick white i-t-multiple" style="margin:0 auto"></div>'; //复选列 test
		...
		ps_obj.find('input:text:not(.no-border), textarea:not(.no-border)').each(function(){
			...
			if(type != '' && type != null && typeof type != 'undefined'){
				if(type.indexOf('date') >= 0 ){ //日期类型
					if(typeof meuiCalendar == 'undefined'){
						console.log('“' + wenzi + '”是日期类型，需请引入日历控件：jquery.jedate.js, meui-calendar.js');
						return false;
					}
					if(typeof meuiCalendar.jeCalendar != 'function') return false;
					meuiCalendar.jeCalendar($this);
				}
			}
		})
	}
	...
	*/
	...
	var showAnimate = function(ps_str){
		if(typeof meui != 'undefined') {
			if(typeof meui.showAnimate === 'function') meui.showAnimate(ps_str);
		}
	};

	var destroyAnimate = function(){
		if(typeof meui != 'undefined') {
			if(typeof meui.destroyAnimate === 'function') meui.destroyAnimate();
		}
	};
	...
	*/
	...
	/**
	 * 添加私有属性、方法到全局对象中
	 * 这些属性、方法不对外开放，只有插件内部可调用，外部无法修改其值
	 * 内部调用方法：
	 * eg1. $.aaa();
	...
	$.extend({
		...
		privateProperty:{
			excelRootNode:'.meui-excel-table', //EXCEL导出表格时的根节点
			tableRootNode:'.meui-like-table', //表格根节点(用于全局赋值,值会改变)
			tableRootElement:'.meui-like-table', //表格根节点(用于调用,值一直不变)		
			tableNestDepth:1, //表格列表数据json嵌套层次.默认1.(有几个Data就是几层嵌套).
			footerNode:'#meui-pager-foot', //底部根节点
			...
			selectorArray:[], //插件节点数组
			selectorCurrent:null, //当前插件节点
			options:{}, //插件参数
...
var meuiLikeTable = {
	...
	/**
	 * 获取表格表列表数据（即表格表单数据）
	 * @param {object} ps_obj 指定表格根节点对象(当页面只有一张表格时则可选). eg. $('.section-table-area').find('.meui-like-table')
	 */
