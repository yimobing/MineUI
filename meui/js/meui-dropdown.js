...
(function($){
	...
	var dropdownPC = function(){
		...
		var showBlock = function(ev,object){
			...
			var _captionStr = gCaption == '' ? '' : '<div class="meui-drop-down-caption">' + gCaption + '</div>';
			//=====创建节点
			var nodeClassName = gChain ? ' meui-drop-beauty ' + gCloseButtonAppearance : '';
			if(checkIsMobile()) nodeClassName += ' meui-drop-mobile';
			var _ouhtml = '<div class="meui-drop-down'+nodeClassName+'">'+ _captionStr + 
							'<div class="meui-drop-down-list"></div>'+
						'</div>';
			if($('.meui-drop-down').length==0) $('body').append(_ouhtml);

			//=====定位根节点
			var parent = $('.meui-drop-down');
			var child = parent.find('.meui-drop-down-list');
			...
			child.off('click').on('click','li:not(".wrong-tips")',function(e){ //off('click) 先释放掉,不然回调函数可能执行两次
				...
				var colIndex = $(this).parents('.meui-drop-down-column').index();
				//上一个值
			...
			if(settings.showCloseButton) {
				parent.find('.meui-drop-down-close').remove();
				parent.append('<div class="meui-drop-down-close' + ' ' + gCloseButtonAppearance + '">关闭</div>');
				child.addClass('no-box-shadow');
				parent.find('.meui-drop-down-close').on('click',function(){
					parent.remove();
				})
			}
			...
			function createPullDownList(ps_initId, ps_initVal, ps_jsonArr){
				...
				for(var k=0;k<ps_jsonArr.length;k++){
					...
					_inHtml+='<div class="meui-drop-down-column" style="width:' + colW + '%"><ul>';
					//html拼接
					var _locateIndex = 0;
				...
				//重置标题
				if(gCaption != '') parent.find('.meui-drop-down-caption').text(gCaption);
				else parent.find('.meui-drop-down-caption').remove();
				...
				//定位到当前值
				for(var k=0;k<ps_jsonArr.length;k++){
					var _$grand = child.find('.meui-drop-down-column').eq(k).find('ul');
		...
		return {
			init:function(opt){
				//不能在初始化时作全局赋值,不能插件无法多处调用;需在click事件中才进行全局赋值
				//opt = $.extend({},defaults,opt||{});
				//console.log('插件初始化'); //只能用console.log() 不能用 alert()
				//settings = opt; //全局赋
				//默认值赋值
 ...
 * @param {*} obj 对象
-------------------------*/
var meuiDropdown = function(opt,obj){
	opt['responseEvent'] = 'custom';
	$(obj).meuiDropdown(opt);
}