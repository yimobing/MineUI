...
(function ($) {
	$.fn.extend({
		meuiKeyboard: function (options) {

			var defaults = {
				object: null, //当前调用数字键盘的元素对象(返回给前台页面,方便调用) 前台调用方法eg1. this.object[0].id; //当前元素的ID  eg2.this.object[0].className; //当前元素class
				title: '数字键盘',
				mode: '', //键盘模式,默认空(可缺省). mobile 移动端模式, computer 电脑端模式. 插件会根据终端设备自由切换模式, 移动端模式下键盘使用绝对定位, 电脑端模式下键盘使用相对定位, 某些情况下,在移动端强制使用电脑端模式效果会更好.
			...
			this.each(function () {
				...
				$this.on(mouseEvent, function (event) { //edit 20190606-1 本行 
					...
					$('.meui-keyboard-layer').remove(); //关闭数字键盘(必须!)

					var topClass = settings.title == '' ? ' shadow-top' : '';
					//生成数字键盘节点
					var html = '<div class="meui-keyboard-layer' + topClass + '">';
					html += settings.title == '' ? '' : '<div class="form_title">' + settings.title + '</div>';
					...
					html += '</div>';
					if ($('.meui-keyboard-layer').length == 0) {
						$('body').append(html);
					...
					if (!isDeviceMobile() || settings.mode == 'computer') {
						...
						$('.meui-keyboard-layer').attr('style', style);
					}
					...
					var numH = $('.meui-keyboard-layer').get(0).offsetHeight; //数字键盘高度
					...
					$(document).on('click', function (e) {
						...
						//$('.meui-keyboard-layer').remove();
						$('.meui-keyboard-layer').slideUp('fast', function () { //edit 20190605 把其它函数移到 slideUp里面
							$(this).remove();
...
var keyboardUi = {
	...
	*/
	removeCursor: function (obj) {
		...
		if ($('.meui-keyboard-layer').length > 0) keyboardUi.showCursor(obj); //edit 20190605
	},
